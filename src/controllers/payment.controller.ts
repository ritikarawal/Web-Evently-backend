...
export class PaymentController {
  private async notifyPaymentSuccess(params: {
    ...
    const [event, payer] = await Promise.all([
      EventModel.findById(eventId).select("title organizer attendees"),
      UserModel.findById(payerUserId).select("username firstName lastName"),
    ]);
  ...
  async createPayment(req: Request, res: Response) {
    try {
      const body = req.body ?? {};
      const userId = ((req as any).userId || body.userId || "").toString();
      ...
      const existingSuccess = await PaymentModel.findOne({
        userId,
        eventId,
  ...
  async getPaymentsByEvent(req: Request, res: Response) {
    try {
      ...
      const payments = await PaymentModel.find({ eventId })
